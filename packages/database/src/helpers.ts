...
const TYPEORM_CONNECTION: SUPPORTED_DBMS = (process.env.TYPEORM_CONNECTION as SUPPORTED_DBMS) ?? 'postgres';
...
const TYPEORM_SYNCHRONIZE: boolean = ['true', '1'].includes(String(process.env.TYPEORM_SYNCHRONIZE ?? 'true').toLowerCase());
const TYPEORM_LOGGING: boolean = ['true', '1'].includes(String(process.env.TYPEORM_LOGGING ?? 'false').toLowerCase());
